...
export default function CalaCollection() {
  ...
  return (
    <section className="relative w-full bg-white py-10 pl-6 md:pl-10 overflow-hidden">
      {/* Header */}
      <div className="relative z-20 pointer-events-none pr-6 md:pr-10 mb-4">
        <p 
      ...
      <div className="relative mt-7 md:mt-9">
        ...
        {canScrollLeft && (
          <button
            onClick={() => scroll("left")}
            aria-label="Previous"
            className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 flex items-center justify-center bg-white/80 hover:bg-white shadow-md transition-all"
        ...
        {canScrollRight && (
          <button
            ...
            aria-label="Next"
            className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 flex items-center justify-center bg-white/80 hover:bg-white shadow-md transition-all"
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
