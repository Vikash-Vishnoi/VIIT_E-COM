  ...
  return (
    <nav className="hidden lg:flex items-center gap-7 mx-8">
      {navLinks.map((link) => {
        ...
        const hasSubSub = catChildren.some(sub => sub.children.length > 0);
          ...
          <div key={link.label} className={hoverEnabled ? "group" : ""}>
            ...
            <div className={`absolute left-0 right-0 top-full mt-0 w-screen rounded-lg border border-gray-200 bg-white shadow-lg pt-4 opacity-0 translate-y-2 pointer-events-none transition-all duration-200 z-50 before:absolute before:left-0 before:right-0 before:-top-10 before:h-10 before:content-[''] before:block ${hoverEnabled ? "group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto" : ""}`}>
              {hasSubSub ? (
                ...
                <div className="grid gap-6 px-5 pb-5" style={{ gridTemplateColumns: `auto repeat(${catChildren.length + 1}, minmax(0,1fr))` }}>
                  {/* Header image */}
                  {headerImg && (
                  ...
                  {/* Category View All Column */}
                  <div className="min-w-0 flex flex-col justify-start pt-7">
                    <Link
                      href={`/${link.slug}`}
                      onClick={handleLinkClick}
                      className="text-sm font-bold text-black hover:opacity-70 hover:underline underline-offset-4"
                    >
                      View All
                    </Link>
                  </div>
                ...
                <div className="grid grid-cols-[auto_minmax(0,1fr)] gap-6 px-5 pb-5">
                  ...
                  <div className="flex flex-col gap-2">
                    <Link
                      href={`/${link.slug}`}
                      onClick={handleLinkClick}
                      className="text-sm font-bold text-black hover:opacity-70 hover:underline underline-offset-4 whitespace-nowrap mb-1"
                    >
                      View All
                    </Link>
                    {catChildren.map(sub => (
