...
function buildUrl(pathname: string, q: string, sort: FeedSortKey, page: number): string {
...
export default function SearchClientPage({
  ...
  function handleSortChange(value: FeedSortKey) {
    startTransition(() => {
      router.push(buildUrl(pathname, searchQuery, value, 1));
    });
  }
  ...
  function goToPage(page: number) {
    startTransition(() => {
      router.push(buildUrl(pathname, searchQuery, currentSort, page));
  ...
  /* ── Render ──────────────────────────────────────────────────────────── */
  return (
    <div className={`min-h-screen bg-white transition-opacity duration-200 mt-20 md:mt-24 ${isPending ? "opacity-60 pointer-events-none" : ""}`}>
