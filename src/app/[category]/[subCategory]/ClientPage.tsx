...
type Props = {
  ...
  /** Other level-1 cats under the same parent → shown in the tab bar */
  siblings: SubCatSibling[];
  /** Level-2 sub-sub-categories under this sub-cat → shown as secondary pills */
...
export default function SubCategoryClientPage({
  ...
  useEffect(() => {
    const activeEl = document.getElementById(`nav-item-${subCategorySlug}`);
  ...
  return (
    <div className={`min-h-screen bg-white transition-opacity duration-200 ${isPending ? "opacity-60 pointer-events-none" : ""}`}>
      ...
      {/* ── Sibling tab bar (level-1 siblings) ───────────────────────────── */}
      <div className="relative border-b border-gray-100 bg-white">
        ...
        >
          ...
          {/* Sub-sub-category pills — secondary row within the same scrollable bar */}
          ...
          {children.map((child) => (
            <Link
              ...
              className="flex-shrink-0 px-4 py-1.5 rounded-full border border-dashed border-gray-200 text-[11px] tracking-widest uppercase font-semibold text-gray-400 hover:border-gray-400 hover:text-black transition-all duration-200"
            >
              {child.label}
            </Link>
          ))}
